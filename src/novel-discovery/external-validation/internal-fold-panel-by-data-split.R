...
run.ext <- function(train.count, train.sample_info, test.count, test.sample_info) {
  ...
  pred.df <- data.frame(matrix(data=NA, nrow=nrow(train.count), ncol=5))
  ...
  for (i in 1:length(folds)) {
    ...
    test.count.fold <- train.count[fold,,drop=F]
    test.sample.info.fold <- train.sample_info[fold,]
    ...
    if (class.problem == "hcg") {
      ...
      val.y <- test.sample.info.fold$hCG_Result
    } else if (class.problem == "lbo") {
      train.y <- train.sample.info.fold$outcome_preg
      val.y <- test.sample.info.fold$outcome_preg
    }
    ...
    if (model.to.use == "ridge") {
      ...
      val.matrix <- model.matrix(~.-1, test.count.fold)
      ...
      model <- train(x=train.count.fold,
                        ...
                        method="rf",
                        metric="ROC")
      val.preds <- predict(model, test.count.fold, type='prob')[,2]
    }
    # Create a new dataframe that we will append to pred.df.
    save.df <- data.frame("sample.id"=test.sample.info.fold$sample_id,
                          "parent"=test.sample.info.fold$OPU_nr,
                          "true"=as.character(val.y),
                          "pred"=val.preds,
                          "fold"=i)
    pred.df[fold,] <- save.df
  }
  
  # Convert the true class labels back to a factor.
  if (class.problem == "hcg") {
    pred.df$true <- factor(pred.df$true, levels=c("Negative", "Positive"))
  } else {
    pred.df$true <- factor(pred.df$true, levels=c("NPO", "Live_birth"))
  }
  
  # Return the result from all of the folds.
  # Note that $model is not used in downstream analyses
  list(preds=pred.df,
       model=model,
...
out.dir <- file.path('results', 'kfold-panel-by-validation-split-internal', val.config, folder.name)
