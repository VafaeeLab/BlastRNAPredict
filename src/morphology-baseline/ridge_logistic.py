...
# ── 2. Parse Emb_stage into 3 ordinal features ───────────────────────────────
GRADE_MAP = {'A': 1, 'B': 2, 'C': 3}

def parse_emb_stage(stage):
    if pd.isna(stage):
        return np.nan, np.nan, np.nan
    s = str(stage).strip()
    expansion = int(s[0])                    # numeric (3, 4, 5)
    icm       = GRADE_MAP.get(s[1], np.nan) # A=1, B=2
    te        = GRADE_MAP.get(s[2], np.nan) # A=1, B=2
    return expansion, icm, te

df[['Expansion', 'ICM', 'TE']] = df['Emb_stage'].apply(
    lambda x: pd.Series(parse_emb_stage(x))
)
...
train = df[df['Cohort'] == 'Training'].dropna(subset=['Expansion', 'ICM', 'TE', 'outcome'])
test  = df[df['Cohort'] == 'Validation'].dropna(subset=['Expansion', 'ICM', 'TE', 'outcome'])

FEATURES = ['Expansion', 'ICM', 'TE']
...
# ── 5. Ridge logistic regression — λ via 10-fold CV (binomial deviance) ──────
Cs = np.logspace(-4, 4, 200)          # C = 1/λ

clf_cv_search = LogisticRegressionCV(
    Cs        = Cs,
    cv        = 10,
    penalty   = 'l2',
    scoring   = 'neg_log_loss',        # = minimise binomial deviance
    solver    = 'lbfgs',
    max_iter  = 5000,
    random_state = 42
)
...
for name, coef in zip(FEATURES, clf_cv_search.coef_[0]):
    print(f"  {name:10s}: {coef:.4f}")

# ── 6. Internal cross-validated metrics (10-fold, using optimal C) ────────────
